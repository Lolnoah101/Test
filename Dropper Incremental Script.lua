...
local treeUpgradesEnabled = false

local Toggle = Tab:AddToggle({
    Name = "Tree Unlocks",
    Default = false,
    Callback = function(Value)
        treeUpgradesEnabled = Value
        
        if treeUpgradesEnabled then
            task.spawn(function()
                while treeUpgradesEnabled do
                    ...
                    local remotes = game:GetService("ReplicatedStorage").Remotes
                    local treeUpgrade = remotes:WaitForChild("TreeUpgrade")
                    
                    for _, upgrade in ipairs(upgrades) do
                        treeUpgrade:FireServer(upgrade)
                        task.wait(0.6)   -- ← adjust this: 0.4–1.2 seconds usually works
                    end
                    
                    task.wait(10)   -- your original big wait after the full cycle
                end
            end)
        end
    end
